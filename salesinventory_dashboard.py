...
if uploaded_file:
    ...
    default_path = "khenmergeddata.xlsx"
    if os.path.exists(default_path):
        merged_data = load_data(default_path)
        st.info(" Using default file: khenmergeddata.xlsx")
    else:
...
if date_range:
    filtered_data = filtered_data[(filtered_data['ORDERDATE'] >= pd.to_datetime(date_range[0])) & 
                                  (filtered_data['ORDERDATE'] <= pd.to_datetime(date_range[1]))]
if product_line:
...
fig3 = px.bar(monthly_sales, x='YearMonth', y='SALES', color='Anomaly',
              color_discrete_map={"High": "red", "Low": "blue", "Normal": "green"})
